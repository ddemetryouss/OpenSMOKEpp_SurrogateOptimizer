...
list_species = { 'NC7H16', 'NC10H22', 'NC12H26', 'NC16H34', ...
                 'IC8H18', 'IC16H34', ...
                 'MCH', 'DECALIN', ...
                 'T124MBZ', 'O-XYL', 'BBZ' };
...
target.HC  = 1.194;         % H/C ratio (experiments)
target.MW  = 154.;          % Molecular weight (kg/kmol experiments)
target.CN  = 47.;           % Cetane number (experiments)
target.TSI = 50.;           % Threshold Soot Index (experiments)
target.mu = 2.6672;         % Viscosity (cP @298K, experiments)
target.YSI = 60;            % Yield Sooting Index (experiments)
target.rho = 700;           % Density

% Viscosity
target.Tvis = 253.;         % Temperature at which viscosity is evaluated (in K)
...
target.ln = [1 1 1 1];                          % minimum number of species per family
target.un = [4 2 2 3];                          % maximum number of species per family
target.nmax = 11;                                % maximum number of species (total)
target.minx = 0.;                               % minimum mole fractions for individual component
target.v_constraints = true;                   % constraints on min/max amounts of families (volume fractions)
target.lv = [0.10 0.20 0.20 0.10];              % min amounts of families (volume fractions)
target.uv = [0.30 0.40 0.35 0.25];              % max amounts of families (volume fractions)
...
optimizer.weight_rho = 0.;          % density
...
fprintf(' * mu:  Target =%8.3f - Solution =%8.3f\n', target.mu,  solution.mu);
fprintf(' * YSI: Target =%8.3f - Solution =%8.3f\n', target.YSI, solution.YSI);
fprintf(' * rho: Target =%8.3f - Solution =%8.3f\n', target.rho, solution.rho);
fprintf('------------------------------------------------------------------\n');
fprintf('\n');
